...
This is the advanced, step-by-step installation path. Prefer the automated
[Getting Started](setup.md) workflow for a supported first-time deployment.
...
```bash
cd ~/repos/infra
task bootstrap:init-local-config
task docker:check-host
```
