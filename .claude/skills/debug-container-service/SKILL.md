...
- The service name in `category/service-name` format (e.g. `ai/open-webui`, `security/traefik`, `media/video/jellyfin`)
...
- **Container name** (needed for `docker logs` — usually set explicitly as `container_name:`)
...
1. Get the host-side volume paths for the container:
   ```bash
   docker inspect <container_name> --format '{{range .Mounts}}{{.Source}}{{"\n"}}{{end}}'
   ```
2. Look for log files in those paths:
   ```bash
